class Solution:
    ...
    def findMedianSortedArrays(self, nums1: List[int], nums2: List[int]) -> float:
        ...
        if (lenA + lenB) % 2 == 1:
            return self.getKth(nums1, lenA, nums2, lenB, (lenTotal + 1) // 2, 0, 0)
        else:
            x = self.getKth(nums1, lenA, nums2, lenB, lenTotal // 2, 0, 0)
            y = self.getKth(nums1, lenA, nums2, lenB, lenTotal // 2 + 1, 0, 0)
            return (x + y) / 2
